...
from discogs_rec_api.dependencies import releases_repository, get_db
...
async def get_styles(
    ...
    releases_repository: ReleasesRepository = Depends(releases_repository),
    ...
    Args:
        ...
        releases_repository: Releases CRUD operations dependency
        db: Database session dependency

    Returns:
        dict: Paginated list of unique musical styles
    """
    result = await releases_repository.get_styles(page=page, limit=limit, db=db)
    return result
...
async def get_artists(
    ...
    """
    result = await releases_repository.get_artists(page=page, limit=limit, db=db)
    return result
...
async def get_labels(
    ...
    """
    result = await releases_repository.get_labels(page=page, limit=limit, db=db)
    return result
...
async def get_countries(
    ...
    """
    result = await releases_repository.get_countries(page=page, limit=limit, db=db)
    return result
...
async def get_year_range(
    ...
    """
    result = await releases_repository.get_year_range(db=db)
    return result
...
async def get_release(
    ...
    """
    try:
        result = await releases_repository.get_release_by_id(
            db=db, release_id=release_id
        )
        return result
...
async def get_releases(
    ...
    """
    filters = {
        k: v for k, v in locals().items() if k not in ["releases_repository", "db"]
    }
    result = await releases_repository.get_releases(db=db, filters=filters)
    return result
